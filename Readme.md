# Báo cáo Audit Mapping Unit Test ↔ Thiết kế (Pd)

- Dự án: Humming Vue R2 ventilator – Pd (Patient Data) module
- Phạm vi audit: ALL
...
- Phân tích UTS/SDD theo Pandoc JSON AST; trích xuất Design Unit theo mẫu header bảng (`Unit ID` trong UTS, `ID:` trong SDD).
- Trích xuất đường dẫn file tham chiếu bằng regex cho chuỗi `Pd/...` và `Test/Pd/...` trong các bảng.
- Quét `Test/Pd` để đếm GoogleTest macros (`TEST`, `TEST_F`, `TEST_P`) sau khi loại bỏ comment theo best-effort (giữ số dòng).
- Tìm kiếm token hàm (qualified) từ UTS/SDD trong các source file mà UTS liệt kê; nếu không thấy thì tìm tiếp trong `Pd` để phát hiện lệch baseline spec↔repo (best-effort substring match; không build/ctags).
- Không chạy unit test; không đưa ra kết luận pass/fail hoặc coverage.

## Thống kê Test Case (Unit Test Pd)

### A) Tổng quan repository (quét `Test/Pd`)

- Tổng số file unit test đã quét: 14
- Tổng số test case tìm được (TEST/TEST_F/TEST_P): 400
- Tổng số INSTANTIATE_TEST_*_P phát hiện được: 0

### B) Mức độ mapping (UTS → Design Unit)

- Tổng số Design Unit trong phạm vi: 13
- Số unit có ≥1 test case được map (mức file): 13
- Số unit có 0 test case được map: (none)
- Tổng số test case được map (không double-count; mức file): 399
- Tổng số test case map bị mơ hồ do dùng chung file: 399
- Tổng số test case chưa được map (có trong repo nhưng UTS không tham chiếu): 1
  - Trong các file test mà UTS tham chiếu (mức test-case, mapping theo token best-effort):
    - Map mạnh (chỉ khớp đúng 1 Design Unit): 252
    - Mơ hồ (khớp ≥2 Design Unit): 20
    - Chưa gán (khớp 0 Design Unit): 127
- Top file test chưa được map theo số test case: Test/Pd/PtDataTest/PtDataTest (copy).cpp(1)
...
### D) Top 10 file test theo số test case (Phụ lục)
...
- Mức độ: MAJOR
- DesignUnitID: N/A
- Vấn đề: Repository contains Pd unit test files not referenced by UTS (orphan/untraceable tests).
- Bằng chứng: Repo scan: 1 orphan files under Test/Pd; top: Test/Pd/PtDataTest/PtDataTest (copy).cpp(1)
- Hành động khuyến nghị (audit-ready):
...
- Tóm tắt khắc phục: Update UTS to reference these test files and map them to Design Units, or remove/relocate tests that are out-of-scope.

### F-002

- Mức độ: MAJOR
- DesignUnitID: MULTI
- Vấn đề: Same unit test file is referenced by multiple Design Units; mapping is ambiguous at test-case level.
- Bằng chứng: UTS-referenced file `Test/Pd/PdAlarmsTest/PdAlarmsTest.cpp` mapped to DesignUnitIDs: DES-MAIN-PD-AL01, DES-MAIN-PD-AL02, DES-MAIN-PD-AL03, DES-MAIN-PD-AL04.
- Hành động khuyến nghị (audit-ready):
...
- Tóm tắt khắc phục: Add per-test trace tags in code (e.g., comments with UTS Test Case IDs) or split tests per Design Unit and update UTS accordingly.
...
- DesignUnitID: MULTI
- Vấn đề: Same unit test file is referenced by multiple Design Units; mapping is ambiguous at test-case level.
- Bằng chứng: UTS-referenced file `Test/Pd/PdTask/PdTaskTest.cpp` mapped to DesignUnitIDs: DES-MAIN-PD-PT01, DES-MAIN-PD-PT02, DES-MAIN-PD-PT03, DES-MAIN-PD-PT04.
- Hành động khuyến nghị (audit-ready):
  - Choose one: (A) split the shared test file into per-Design-Unit test files, or (B) add per-test trace tags in code that reference UTS Test Case IDs.
  - Update UTS to map each Design Unit to either a dedicated test file or explicit gtest case names.
  - Re-run this audit to confirm StrongMappedCount increases and ambiguity decreases.
...
- DesignUnitID: MULTI
- Vấn đề: Same unit test file is referenced by multiple Design Units; mapping is ambiguous at test-case level.
- Bằng chứng: UTS-referenced file `Test/Pd/PtDataTest/PtDataTest.cpp` mapped to DesignUnitIDs: DES-MAIN-PD-PE01, DES-MAIN-PD-PE02, DES-MAIN-PD-PE03, DES-MAIN-PD-PE04, DES-MAIN-PD-PE05.
- Hành động khuyến nghị (audit-ready):
  - Choose one: (A) split the shared test file into per-Design-Unit test files, or (B) add per-test trace tags in code that reference UTS Test Case IDs.
  - Update UTS to map each Design Unit to either a dedicated test file or explicit gtest case names.
  - Re-run this audit to confirm StrongMappedCount increases and ambiguity decreases.
...
- Vấn đề: Qualified function(s) referenced by UTS/SDD are not found in UTS-listed source file(s) but exist elsewhere in Pd (UTS↔repo file mapping mismatch).
- Bằng chứng: UTS source evidence: UTS:blocks[82]/Table/Body/Row[1]/Col[1] (ファイル名/File); SDD function evidence: SDD:blocks[99]/Table/Body/Row[4]/Col[1] (関数 / Function：); Repo search: Ptdata::PdSpontaneous:MISMATCH(similar-name 'PdSpontaneous' found Pd/ptdata.cpp:307, qualified symbol not found);Ptdata::BuildExhMsg:Pd/ptdata.cpp:2510;Ptdata::BuildInhMsg:Pd/ptdata.cpp:2425;Ptdata::CompExhTidalVolumeInAPRV:Pd/ptdata.cpp:385
- Hành động khuyến nghị (audit-ready):
...
- Tóm tắt khắc phục: Update UTS source-file list to include the actual implementing file(s) and/or adjust SDD function listing; prefer linking each UTS Test Case ID to the concrete gtest that covers it.
...
- Vấn đề: Qualified function(s) referenced by UTS/SDD are not found in UTS-listed source file(s) but exist elsewhere in Pd (UTS↔repo file mapping mismatch).
- Bằng chứng: UTS source evidence: UTS:blocks[90]/Table/Body/Row[1]/Col[1] (ファイル名/File); SDD function evidence: SDD:blocks[115]/Table/Body/Row[4]/Col[1] (関数 / Function：); Repo search: PdTask::TestLogFileWriting:MISMATCH(found Pd/pdtask.cpp:206 not in UTS SourceFile_UTS list)
- Hành động khuyến nghị (audit-ready):
  - Update UTS SourceFile_UTS list to include the actual implementing file(s) indicated by repo evidence, or update SDD function naming to match the repository.
  - For renamed/mismatched symbols, record the rationale (rename history) and update SDD/UTS accordingly.
  - Add a traceability mechanism: comment tags containing UTS Test Case IDs near each gtest case (or split tests per unit).
...
- Vấn đề: Qualified function(s) referenced by UTS/SDD are not found in UTS-listed source file(s) but exist elsewhere in Pd (UTS↔repo file mapping mismatch).
- Bằng chứng: UTS source evidence: UTS:blocks[74]/Table/Body/Row[1]/Col[1] (ファイル名/File); SDD function evidence: SDD:blocks[86]/Table/Body/Row[4]/Col[1] (関数 / Function：); Repo search: Ptdata::ClearHistory:MISMATCH(found Pd/ptdata.cpp:2236 not in UTS SourceFile_UTS list);PdTask::TestLogFileWriting:Pd/pdtask.cpp:206;PdTask::run:Pd/pdtask.cpp:42
- Hành động khuyến nghị (audit-ready):
  - Update UTS SourceFile_UTS list to include the actual implementing file(s) indicated by repo evidence, or update SDD function naming to match the repository.
  - For renamed/mismatched symbols, record the rationale (rename history) and update SDD/UTS accordingly.
  - Add a traceability mechanism: comment tags containing UTS Test Case IDs near each gtest case (or split tests per unit).
...
- Vấn đề: Qualified function(s) referenced by UTS/SDD are not found in UTS-listed source file(s) but exist elsewhere in Pd (UTS↔repo file mapping mismatch).
- Bằng chứng: UTS source evidence: UTS:blocks[76]/Table/Body/Row[1]/Col[1] (ファイル名/File); SDD function evidence: SDD:blocks[89]/Table/Body/Row[4]/Col[1] (関数 / Function：); Repo search: PdTask::S_SetEventFlag:MISMATCH(found Pd/pdtask.h:115 not in UTS SourceFile_UTS list);PdTask::initMsgQueue:Pd/pdtask.cpp:226
- Hành động khuyến nghị (audit-ready):
  - Update UTS SourceFile_UTS list to include the actual implementing file(s) indicated by repo evidence, or update SDD function naming to match the repository.
  - For renamed/mismatched symbols, record the rationale (rename history) and update SDD/UTS accordingly.
  - Add a traceability mechanism: comment tags containing UTS Test Case IDs near each gtest case (or split tests per unit).
...
- Vấn đề: Qualified function(s) referenced by UTS/SDD are not found in UTS-listed source file(s) but exist elsewhere in Pd (UTS↔repo file mapping mismatch).
- Bằng chứng: UTS source evidence: UTS:blocks[78]/Table/Body/Row[1]/Col[1] (ファイル名/File); SDD function evidence: SDD:blocks[93]/Table/Body/Row[4]/Col[1] (関数 / Function：); Repo search: Ptdata::ClearHistory:MISMATCH(found Pd/ptdata.cpp:2236 not in UTS SourceFile_UTS list);PdTask::run:Pd/pdtask.cpp:42
- Hành động khuyến nghị (audit-ready):
  - Update UTS SourceFile_UTS list to include the actual implementing file(s) indicated by repo evidence, or update SDD function naming to match the repository.
  - For renamed/mismatched symbols, record the rationale (rename history) and update SDD/UTS accordingly.
  - Add a traceability mechanism: comment tags containing UTS Test Case IDs near each gtest case (or split tests per unit).
...
- Mức độ: MAJOR
- DesignUnitID: DES-MAIN-PD-PE05
- Vấn đề: No evidence-based unit test cases mapped to this Design Unit (0 strong, 0 ambiguous) within UTS-referenced test file(s).
...
- Hành động khuyến nghị (audit-ready):
  ...
  - For renamed/mismatched symbols, record the rationale (rename history) and update SDD/UTS accordingly.
  - Add a traceability mechanism: comment tags containing UTS Test Case IDs near each gtest case (or split tests per unit).
- Tóm tắt khắc phục: Update UTS to reference the correct test file(s) and/or add unit tests that exercise the Design Unit’s functions; add explicit per-test trace tags to link UTS Test Case IDs ↔ gtest cases.

### F-012

- Mức độ: MAJOR
- DesignUnitID: DES-MAIN-PD-PT01
- Vấn đề: All detected test-case matches are ambiguous; no uniquely traceable unit tests for this Design Unit.
...
- Hành động khuyến nghị (audit-ready):
  ...
  - For renamed/mismatched symbols, record the rationale (rename history) and update SDD/UTS accordingly.
  - Add a traceability mechanism: comment tags containing UTS Test Case IDs near each gtest case (or split tests per unit).
- Tóm tắt khắc phục: Add per-test trace tags (UTS Test Case IDs in code) and/or split shared test files per Design Unit; then update UTS to disambiguate mapping at test-case level.
...
- Vấn đề: Unit name differs between UTS and SDD for the same Design Unit ID.
- Bằng chứng: UTS Unit='PdAlarms - ThresholdChecks' vs SDD Unit='ThresholdChecks'.
- Hành động khuyến nghị (audit-ready):
  - Align naming (UTS unit titles vs SDD unit names) to reduce review ambiguity; if intentional, document rationale.
- Tóm tắt khắc phục: Align unit naming between UTS and SDD (or document rationale).
...
- Vấn đề: Unit name differs between UTS and SDD for the same Design Unit ID.
- Bằng chứng: UTS Unit='PdAlarms - Debounce' vs SDD Unit='Debounce'.
- Hành động khuyến nghị (audit-ready):
  - Align naming (UTS unit titles vs SDD unit names) to reduce review ambiguity; if intentional, document rationale.
- Tóm tắt khắc phục: Align unit naming between UTS and SDD (or document rationale).
...
- Vấn đề: Unit name differs between UTS and SDD for the same Design Unit ID.
- Bằng chứng: UTS Unit='PdAlarms - DwellTimers' vs SDD Unit='DwellTimers'.
- Hành động khuyến nghị (audit-ready):
  - Align naming (UTS unit titles vs SDD unit names) to reduce review ambiguity; if intentional, document rationale.
- Tóm tắt khắc phục: Align unit naming between UTS and SDD (or document rationale).
...
- Vấn đề: Unit name differs between UTS and SDD for the same Design Unit ID.
- Bằng chứng: UTS Unit='PdAlarms - AlarmPublisher / O2 Alarms' vs SDD Unit='AlarmPublisher / O2 Alarms'.
- Hành động khuyến nghị (audit-ready):
  - Align naming (UTS unit titles vs SDD unit names) to reduce review ambiguity; if intentional, document rationale.
- Tóm tắt khắc phục: Align unit naming between UTS and SDD (or document rationale).
...
- DesignUnitID: DES-MAIN-PD-PE01
- Vấn đề: Unit name differs between UTS and SDD for the same Design Unit ID.
- Bằng chứng: UTS Unit='Ptdata - BreathPhaseProcessor' vs SDD Unit='BreathPhaseProcessor'.
- Hành động khuyến nghị (audit-ready):
  - Align naming (UTS unit titles vs SDD unit names) to reduce review ambiguity; if intentional, document rationale.
- Tóm tắt khắc phục: Align unit naming between UTS and SDD (or document rationale).
...
- Vấn đề: Unit name differs between UTS and SDD for the same Design Unit ID.
- Bằng chứng: UTS Unit='Ptdata - O2Limits' vs SDD Unit='O2Limits'.
- Hành động khuyến nghị (audit-ready):
  - Align naming (UTS unit titles vs SDD unit names) to reduce review ambiguity; if intentional, document rationale.
- Tóm tắt khắc phục: Align unit naming between UTS and SDD (or document rationale).
...
- DesignUnitID: DES-MAIN-PD-PE03
- Vấn đề: Unit name differs between UTS and SDD for the same Design Unit ID.
- Bằng chứng: UTS Unit='Ptdata - MetricsCalculator' vs SDD Unit='MetricsCalculator'.
- Hành động khuyến nghị (audit-ready):
  - Align naming (UTS unit titles vs SDD unit names) to reduce review ambiguity; if intentional, document rationale.
- Tóm tắt khắc phục: Align unit naming between UTS and SDD (or document rationale).
...
- Vấn đề: Unit name differs between UTS and SDD for the same Design Unit ID.
- Bằng chứng: UTS Unit='Ptdata - GuiPublisher' vs SDD Unit='GuiPublisher'.
- Hành động khuyến nghị (audit-ready):
  - Align naming (UTS unit titles vs SDD unit names) to reduce review ambiguity; if intentional, document rationale.
- Tóm tắt khắc phục: Align unit naming between UTS and SDD (or document rationale).
...
- DesignUnitID: DES-MAIN-PD-PE05
- Vấn đề: Unit name differs between UTS and SDD for the same Design Unit ID.
- Bằng chứng: UTS Unit='Ptdata - LogPublisher' vs SDD Unit='LogPublisher'.
- Hành động khuyến nghị (audit-ready):
  - Align naming (UTS unit titles vs SDD unit names) to reduce review ambiguity; if intentional, document rationale.
- Tóm tắt khắc phục: Align unit naming between UTS and SDD (or document rationale).
...
- DesignUnitID: DES-MAIN-PD-PT01
- Vấn đề: Unit name differs between UTS and SDD for the same Design Unit ID.
- Bằng chứng: UTS Unit='PdTask - Event Loop Unit' vs SDD Unit='EventLoop'.
- Hành động khuyến nghị (audit-ready):
  - Align naming (UTS unit titles vs SDD unit names) to reduce review ambiguity; if intentional, document rationale.
- Tóm tắt khắc phục: Align unit naming between UTS and SDD (or document rationale).
...
- DesignUnitID: DES-MAIN-PD-PT02
- Vấn đề: Unit name differs between UTS and SDD for the same Design Unit ID.
- Bằng chứng: UTS Unit='PdTask - IPC Receive Unit (message gateway)' vs SDD Unit='IPCReceive'.
- Hành động khuyến nghị (audit-ready):
  - Align naming (UTS unit titles vs SDD unit names) to reduce review ambiguity; if intentional, document rationale.
- Tóm tắt khắc phục: Align unit naming between UTS and SDD (or document rationale).
...
- DesignUnitID: DES-MAIN-PD-PT03
- Vấn đề: Unit name differs between UTS and SDD for the same Design Unit ID.
- Bằng chứng: UTS Unit='PdTask - Dispatcher Unit' vs SDD Unit='Dispatcher'.
- Hành động khuyến nghị (audit-ready):
  - Align naming (UTS unit titles vs SDD unit names) to reduce review ambiguity; if intentional, document rationale.
- Tóm tắt khắc phục: Align unit naming between UTS and SDD (or document rationale).
...
- DesignUnitID: DES-MAIN-PD-PT04
- Vấn đề: Unit name differs between UTS and SDD for the same Design Unit ID.
- Bằng chứng: UTS Unit='PdTask - Mode Status Unit' vs SDD Unit='ModeStatus'.
- Hành động khuyến nghị (audit-ready):
  - Align naming (UTS unit titles vs SDD unit names) to reduce review ambiguity; if intentional, document rationale.
- Tóm tắt khắc phục: Align unit naming between UTS and SDD (or document rationale).
...
- Mức độ: OBSERVATION
- DesignUnitID: N/A
- Vấn đề: UTS refers to an external traceability matrix; SRS↔UTS traceability not verifiable from provided inputs alone.
- Bằng chứng: UTS contains 'Refer to Traceability matrix' text; no direct SRS requirement IDs were detected in UTS by simple string search.
- Hành động khuyến nghị (audit-ready):
  - Provide the referenced external traceability matrix and baseline-link it to UTS/SDD and repo revision.
- Tóm tắt khắc phục: Provide the referenced traceability matrix artifact and ensure it is configuration-controlled with the same baseline as UTS/SDD and repo revision.
...
- Số Test Case ID được liệt kê trong UTS: 10 (sample: DES-MAIN-PD-AL01-TC01, DES-MAIN-PD-AL01-TC02, DES-MAIN-PD-AL01-TC03, DES-MAIN-PD-AL01-TC04, DES-MAIN-PD-AL01-TC05)
...
**Bằng chứng từ repository (best-effort)**
...
- Kết quả tìm hàm (token:file:line): PdAlarms::CheckHiPip:Pd/pdalarms.cpp:631;PdAlarms::CheckHiPipEndExh:Pd/pdalarms.cpp:662

**Mapping mức test-case (best-effort, trong các file test UTS tham chiếu)**

- Test case map mạnh (khớp duy nhất): 10
...
- Test case khớp mơ hồ (khớp nhiều unit): 0
- Test case chưa gán trong (các) file test mà unit này tham chiếu: 26 (see report section C totals)
...
- Số Test Case ID được liệt kê trong UTS: 22 (sample: DES-MAIN-PD-AL02-TC01, DES-MAIN-PD-AL02-TC02, DES-MAIN-PD-AL02-TC03, DES-MAIN-PD-AL02-TC04, DES-MAIN-PD-AL02-TC05)
...
- `Test/Pd/PdAlarmsTest/PdAlarmsTest.cpp` tồn tại: OK
  - TEST macros found: 190; sample: PdAlarmsFixture.CheckHiCondition_Activates_After_Reaching_Sensitivity_NonConsecutive@69, PdAlarmsFixture.CheckHiCondition_DoesNotActivate_Before_Sensitivity@105, PdAlarmsFixture.CheckHiCondition_StaysActive_On_ContinuedHighs_And_Clears_On_Normal@122, PdAlarmsFixture.CheckHiCondition_LimitEquality_IsNotHigh@146, PdAlarmsFixture.CheckHiCondition_Sensitivity1_ActivatesImmediately@161, PdAlarmsFixture.CheckHiCondition_StaticCounter_SharedAcross_Alarms@179, PdAlarmsFixture.CheckHiCondition_NormalWhileInactive_DoesNotResetCounter@203, PdAlarmsFixture.CheckLowCondition_Activates_After_Reaching_Sensitivity_NonConsecutive@231
- Kết quả tìm hàm (token:file:line): PdAlarms::CheckHiCondition:Pd/pdalarms.cpp:105;PdAlarms::CheckLowCondition:Pd/pdalarms.cpp:152;PdAlarms::CheckLowPip:Pd/pdalarms.cpp:696

**Mapping mức test-case (best-effort, trong các file test UTS tham chiếu)**

- Test case map mạnh (khớp duy nhất): 22
...
- Số Test Case ID được liệt kê trong UTS: 138 (sample: DES-MAIN-PD-AL03-TC001, DES-MAIN-PD-AL03-TC002, DES-MAIN-PD-AL03-TC003, DES-MAIN-PD-AL03-TC004, DES-MAIN-PD-AL03-TC005)
...
- Số Test Case ID được liệt kê trong UTS: 20 (sample: DES-MAIN-PD-AL04-TC01, DES-MAIN-PD-AL04-TC02, DES-MAIN-PD-AL04-TC03, DES-MAIN-PD-AL04-TC04, DES-MAIN-PD-AL04-TC05)
...
- Test case map mạnh (khớp duy nhất): 20
...
- Số Test Case ID được liệt kê trong UTS: 36 (sample: DES-MAIN-PD-PE01-TC01, DES-MAIN-PD-PE01-TC02, DES-MAIN-PD-PE01-TC03, DES-MAIN-PD-PE01-TC04, DES-MAIN-PD-PE01-TC05)
...
- Kết quả tìm hàm (token:file:line): Ptdata::PdSpontaneous:MISMATCH(similar-name 'PdSpontaneous' found Pd/ptdata.cpp:307, qualified symbol not found);Ptdata::BuildExhMsg:Pd/ptdata.cpp:2510;Ptdata::BuildInhMsg:Pd/ptdata.cpp:2425;Ptdata::CompExhTidalVolumeInAPRV:Pd/ptdata.cpp:385;Ptdata::CompNumBreaths:Pd/ptdata.cpp:286;Ptdata::DoAPRV_PHigh:Pd/ptdata.cpp:2146;Ptdata::DoAPRV_PLow:Pd/ptdata.cpp:1966;Ptdata::DoEndExhPSInAPRV:Pd/ptdata.cpp:3781;Ptdata::DoEndInhExtra:Pd/ptdata.cpp:4119;Ptdata::DoEndInhPSInAPRV:Pd/ptdata.cpp:3898;Ptdata::DoExhalationData:Pd/ptdata.cpp:1689;Ptdata::DoInhalationData:Pd/ptdata.cpp:1432

**Mapping mức test-case (best-effort, trong các file test UTS tham chiếu)**

- Test case map mạnh (khớp duy nhất): 3
...
- Test case khớp mơ hồ (khớp nhiều unit): 4
...
- Test case chưa gán trong (các) file test mà unit này tham chiếu: 92 (see report section C totals)
...
- Số Test Case ID được liệt kê trong UTS: 4 (sample: DES-MAIN-PD-PE02-TC01, DES-MAIN-PD-PE02-TC02, DES-MAIN-PD-PE02-TC03, DES-MAIN-PD-PE02-TC04)
...
- `Test/Pd/PtDataTest/PtDataTest.cpp` tồn tại: OK
  - TEST macros found: 179; sample: PtDataFixture.StopsImmediatelyOnNoPhase_EmptyHistory@136, PtDataFixture.Sanity_PdSpontaneousMapping@148, PtDataFixture.DirectIndicatorSpontaneous_Increments@156, PtDataFixture.PlateauWithSpontaneousBackup_Increments@170, PtDataFixture.CountsSpontaneousAndPlateauBackupCorrectly@184, PtDataFixture.BreaksOnNoPhaseMidway@209, PtDataFixture.ContinuesWhenJustUnderTimeLimit@222, PtDataFixture.StopsWhenTimeLimitReachedOrExceeded@240
- Kết quả tìm hàm (token:file:line): Ptdata::BuildO2Msg:Pd/ptdata.cpp:2563;Ptdata::DoO2:Pd/ptdata.cpp:1790

**Mapping mức test-case (best-effort, trong các file test UTS tham chiếu)**

- Test case map mạnh (khớp duy nhất): 4
...
- Test case khớp mơ hồ (khớp nhiều unit): 1
...
- Số Test Case ID được liệt kê trong UTS: 88 (sample: DES-MAIN-PD-PE03-TC01, DES-MAIN-PD-PE03-TC02, DES-MAIN-PD-PE03-TC03, DES-MAIN-PD-PE03-TC04, DES-MAIN-PD-PE03-TC05)
...
- Số Test Case ID được liệt kê trong UTS: 41 (sample: DES-MAIN-PD-PE04-TC01, DES-MAIN-PD-PE04-TC02, DES-MAIN-PD-PE04-TC03, DES-MAIN-PD-PE04-TC04, DES-MAIN-PD-PE04-TC05)
...
- Test case khớp mơ hồ (khớp nhiều unit): 5
  - `Test/Pd/PtDataTest/PtDataTest.cpp:3675` `PtDataFixture.BuildInhMsg_ProxyReadyUsesActualValues` (candidates: DES-MAIN-PD-PE01, DES-MAIN-PD-PE04)
  - `Test/Pd/PtDataTest/PtDataTest.cpp:3709` `PtDataFixture.BuildInhMsg_ProxyErrorBlanksVolumes` (candidates: DES-MAIN-PD-PE01, DES-MAIN-PD-PE04)
  - `Test/Pd/PtDataTest/PtDataTest.cpp:3772` `PtDataFixture.BuildExhMsg_ProxyReadyCopiesValues` (candidates: DES-MAIN-PD-PE01, DES-MAIN-PD-PE04)
  - `Test/Pd/PtDataTest/PtDataTest.cpp:3815` `PtDataFixture.BuildExhMsg_ProxyErrorBlanksVolumes` (candidates: DES-MAIN-PD-PE01, DES-MAIN-PD-PE04)
...
- `Test/Pd/PtDataTest/PtDataTest.cpp` tồn tại: OK
  - TEST macros found: 179; sample: PtDataFixture.StopsImmediatelyOnNoPhase_EmptyHistory@136, PtDataFixture.Sanity_PdSpontaneousMapping@148, PtDataFixture.DirectIndicatorSpontaneous_Increments@156, PtDataFixture.PlateauWithSpontaneousBackup_Increments@170, PtDataFixture.CountsSpontaneousAndPlateauBackupCorrectly@184, PtDataFixture.BreaksOnNoPhaseMidway@209, PtDataFixture.ContinuesWhenJustUnderTimeLimit@222, PtDataFixture.StopsWhenTimeLimitReachedOrExceeded@240
- Kết quả tìm hàm (token:file:line): PdTask::TestLogFileWriting:MISMATCH(found Pd/pdtask.cpp:206 not in UTS SourceFile_UTS list)

**Mapping mức test-case (best-effort, trong các file test UTS tham chiếu)**

- Test case map mạnh (khớp duy nhất): 0
- Test case khớp mơ hồ (khớp nhiều unit): 0
- Test case chưa gán trong (các) file test mà unit này tham chiếu: 92 (see report section C totals)
...
- Số Test Case ID được liệt kê trong UTS: 4 (sample: DES-MAIN-PD-PT01-TC01, DES-MAIN-PD-PT01-TC02, DES-MAIN-PD-PT01-TC03, DES-MAIN-PD-PT01-TC04)
...
- Kết quả tìm hàm (token:file:line): Ptdata::ClearHistory:MISMATCH(found Pd/ptdata.cpp:2236 not in UTS SourceFile_UTS list);PdTask::TestLogFileWriting:Pd/pdtask.cpp:206;PdTask::run:Pd/pdtask.cpp:42

**Mapping mức test-case (best-effort, trong các file test UTS tham chiếu)**

- Test case map mạnh (khớp duy nhất): 0
- Test case khớp mơ hồ (khớp nhiều unit): 15
...
- Test case chưa gán trong (các) file test mà unit này tham chiếu: 9 (see report section C totals)
...
- Số Test Case ID được liệt kê trong UTS: 6 (sample: DES-MAIN-PD-PT02-TC01, DES-MAIN-PD-PT02-TC02, DES-MAIN-PD-PT02-TC03, DES-MAIN-PD-PT02-TC04, DES-MAIN-PD-PT02-TC05)
...
- `Test/Pd/PdTask/PdTaskTest.cpp` tồn tại: OK
  - TEST macros found: 30; sample: PdTaskFixture.Smoke_ConstructsAndDestroys@220, PdTaskFixture.S_SetEventFlag_Case01_CallsMsgsndWithExpectedArgs@228, PdTaskFixture.S_SetEventFlag_Case02_DiscreteEventFlags@251, PdTaskFixture.S_SetEventFlag_Case08_MsgBufContent@269, PdTaskFixture.S_SetEventFlag_Case15_ErrorPathReturnsNegative@284, PdTaskFixture.S_SetEventFlag_Case16_BoundaryEventFlags@299, PdTaskFixture.S_SetEventFlag_Case18_MultipleCallsIncreaseCount@315, PdTaskFixture.Run_LoopControlledByShim_ErrorPath_TriggersWaitMs@335
- Kết quả tìm hàm (token:file:line): PdTask::S_SetEventFlag:MISMATCH(found Pd/pdtask.h:115 not in UTS SourceFile_UTS list);PdTask::initMsgQueue:Pd/pdtask.cpp:226

**Mapping mức test-case (best-effort, trong các file test UTS tham chiếu)**

- Test case map mạnh (khớp duy nhất): 6
...
- Số Test Case ID được liệt kê trong UTS: 14 (sample: DES-MAIN-PD-PT03-TC01, DES-MAIN-PD-PT03-TC02, DES-MAIN-PD-PT03-TC03, DES-MAIN-PD-PT03-TC04, DES-MAIN-PD-PT03-TC05)
...
- `Test/Pd/PdTask/PdTaskTest.cpp` tồn tại: OK
  - TEST macros found: 30; sample: PdTaskFixture.Smoke_ConstructsAndDestroys@220, PdTaskFixture.S_SetEventFlag_Case01_CallsMsgsndWithExpectedArgs@228, PdTaskFixture.S_SetEventFlag_Case02_DiscreteEventFlags@251, PdTaskFixture.S_SetEventFlag_Case08_MsgBufContent@269, PdTaskFixture.S_SetEventFlag_Case15_ErrorPathReturnsNegative@284, PdTaskFixture.S_SetEventFlag_Case16_BoundaryEventFlags@299, PdTaskFixture.S_SetEventFlag_Case18_MultipleCallsIncreaseCount@315, PdTaskFixture.Run_LoopControlledByShim_ErrorPath_TriggersWaitMs@335
- Kết quả tìm hàm (token:file:line): Ptdata::ClearHistory:MISMATCH(found Pd/ptdata.cpp:2236 not in UTS SourceFile_UTS list);PdTask::run:Pd/pdtask.cpp:42
...
- Số Test Case ID được liệt kê trong UTS: 5 (sample: DES-MAIN-PD-PT04-TC01, DES-MAIN-PD-PT04-TC02, DES-MAIN-PD-PT04-TC03, DES-MAIN-PD-PT04-TC04, DES-MAIN-PD-PT04-TC05)
...
- `Test/Pd/PdTask/PdTaskTest.cpp` tồn tại: OK
  - TEST macros found: 30; sample: PdTaskFixture.Smoke_ConstructsAndDestroys@220, PdTaskFixture.S_SetEventFlag_Case01_CallsMsgsndWithExpectedArgs@228, PdTaskFixture.S_SetEventFlag_Case02_DiscreteEventFlags@251, PdTaskFixture.S_SetEventFlag_Case08_MsgBufContent@269, PdTaskFixture.S_SetEventFlag_Case15_ErrorPathReturnsNegative@284, PdTaskFixture.S_SetEventFlag_Case16_BoundaryEventFlags@299, PdTaskFixture.S_SetEventFlag_Case18_MultipleCallsIncreaseCount@315, PdTaskFixture.Run_LoopControlledByShim_ErrorPath_TriggersWaitMs@335
- Kết quả tìm hàm (token:file:line): PdTask::run:Pd/pdtask.cpp:42

**Mapping mức test-case (best-effort, trong các file test UTS tham chiếu)**

- Test case map mạnh (khớp duy nhất): 0
- Test case khớp mơ hồ (khớp nhiều unit): 2
  - `Test/Pd/PdTask/PdTaskTest.cpp:354` `PdTaskFixture.Run_NonBreathingMode_CallsClearHistory_AndSkipsElseBranch` (candidates: DES-MAIN-PD-PT01, DES-MAIN-PD-PT03, DES-MAIN-PD-PT04)
...
- Độ đầy đủ traceability: resolve MAJOR findings (orphan tests, ambiguous shared files, any missing mappings).
- Tiêu chí khách quan: ensure each UTS test case has objective expectations and is directly traceable to concrete gtest cases.
- Kiểm soát baseline: ensure UTS/SDD/SRS/SAS and repository revision are baseline-linked; update stale names/paths.
- Độ vững của bằng chứng: prefer per-test trace tags (e.g., comments with UTS Test Case IDs) to disambiguate shared test files.
